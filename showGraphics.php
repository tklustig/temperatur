...
if (!empty($_REQUEST['submit0'])) {
    ...
    if (!empty($_REQUEST['date0'])) {
        ...
        $sql = "SELECT id,datum,Temperatur_Celsius FROM temperaturs;";
        $query1 = $DatabaseObject->Abfragen($connection, $sql);
		$sensorIsDefect=false;
        if (is_array($query1)) {
			 //Iteriere über das Array(query1) und überprüfe, ob der Wert der Temperatur 0 ist. Falls ja, ist der Sensor defekt 
			 for($i=0;$i<count($query1);$i++){
				 if($query1[$i]['Temperatur_Celsius'] == 0 && $strDatum == $query1[$i]['datum']){
					 $sensorIsDefect=true;
					 break;
				 }else
					 $sensorIsDefect=false;
			 }
			 //Sofern der Sensor defekt ist, melde es dem User via JavaScript
			 if($sensorIsDefect){
				  ?>
            <script>
                var alertWidth = 300;
                ...
                var yAlertStart = 200;
                var alertTitle = "<p class='pTitle'><b>! Warnung !</b></p>";
                var alertText = "<p class='pAlert'>Der Temperatursensor DHT22 scheint defekt zu sein. Neukauf ist geplant. Wählen Sie ein früheres Datum!</p>";
                showAlert(alertWidth, alertHeight, xAlertStart, yAlertStart, alertTitle, alertText);
            </script>
            <?php
			 }
            ...
            for ($i = 0; $i < count($query1); $i++) {
                ...
                if ($strDatum == $query1[$i]['datum']) {
                    ...
                    break;
                    //andernfalls setze die Session auf 1(offenbart eine Schwäche von PHP: Dem Datentypenmismatching)
                } else
                    $_SESSION['pk'] = 1;
            }
        } else {
