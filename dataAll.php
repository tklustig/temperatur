...
<html> <!-- Definition des Stammverzeichnises -->
    ...
    if (!empty($_REQUEST['submit0'])) {
        if (!empty($_REQUEST['startId'])) {
            ...
            $sql = "SELECT id,datum,uhrzeit,Temperatur_Celsius,ROUND(611.2 * EXP((17.62*Luftfeuchtigkeit_Prozent)/(243.12+Luftfeuchtigkeit_Prozent))/100,2) AS Luftfeuchtigkeit_Prozent,created_at FROM temperaturs WHERE id>=$id LIMIT 49";
    ...
    if (!$boolQuery2)
        $sql = "SELECT id,datum,uhrzeit,Temperatur_Celsius,ROUND(611.2 * EXP((17.62*Luftfeuchtigkeit_Prozent)/(243.12+Luftfeuchtigkeit_Prozent))/100,2) AS Luftfeuchtigkeit_Prozent,created_at FROM temperaturs LIMIT 49";
    else
        $sql = "SELECT id,datum,uhrzeit,Temperatur_Celsius,ROUND(611.2 * EXP((17.62*Luftfeuchtigkeit_Prozent)/(243.12+Luftfeuchtigkeit_Prozent))/100,2) AS Luftfeuchtigkeit_Prozent,created_at FROM temperaturs WHERE id>=$id LIMIT 49";
    $query2 = $DatabaseObject->Abfragen($connection, $sql);
    if (is_array($query2))
